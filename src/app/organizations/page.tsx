...
const problems = [
  { icon: Car, title: "Playing Time & Role Conflict", problem: "A playing-time complaint lands on your desk before the coach even knows there's a concern.", helps: "Parents get a framework for knowing when a conversation is warranted—and how to have it with the coach, not about the coach." },
  ...
  { icon: Compass, title: "Pressure Following Athletes Home", problem: "What happens in the car ride home undoes what happened at practice.", helps: "Parents get the exact language for the fifteen minutes after a hard game—so home reinforces what you're building, not undermines it." },
  ...
  { icon: Users, title: "Misaligned Expectations", problem: "Time, fees, travel, and training create expectations your organization never actually promised.", helps: "Parents get a realistic picture of development early, so expectations are set before they become conflict." },
...
const evidence = [
  ...
  { icon: Users, figure: "[TBD]", body: "Coach turnover / retention statistic — to be sourced.", support: "Coaches who feel supported by parents stay longer.", source: "Pending" },
  { icon: Shield, figure: "[TBD]", body: "Statistic on parent behaviour's effect on official/referee retention or program reputation — to be sourced.", support: "How parents show up shapes whether officials, volunteers, and coaches want to come back.", source: "Pending" },
];
        ...
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {problems.map((p, i) => (
              ...
              >
                <p.icon className="w-6 h-6 text-teal mb-4" strokeWidth={1.5} aria-hidden="true" />
      ...
      <section className="max-w-6xl mx-auto w-full px-6 py-20 md:py-28">
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
          {proactiveBoxes.map((b, i) => (
            ...
            >
              <b.icon className="w-6 h-6 text-teal mb-3" strokeWidth={1.5} aria-hidden="true" />
        ...
        <motion.div {...fadeUp} className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-3xl mx-auto mt-14">
          <Placeholder label="Phone: Situation Select" className="aspect-[9/16] rounded-xl" />
          <Placeholder label="Phone: Recommended Resource" className="aspect-[9/16] rounded-xl" />
          <Placeholder label="Phone: Exact Script" className="aspect-[9/16] rounded-xl" />
        </motion.div>
      </section>
            ...
            <div className="grid grid-cols-2 gap-4">
              {portalBenefits.map((b) => (
                <div key={b.title} className="bg-cream border border-border-grey rounded-2xl p-5">
                  <b.icon className="w-5 h-5 text-teal mb-2" strokeWidth={1.5} aria-hidden="true" />
            ...
            {alignmentPoints.map((v) => (
              <div key={v.label}>
                <div className="flex items-center gap-2 mb-1">
                  <v.icon className="w-5 h-5 text-teal shrink-0" strokeWidth={1.5} aria-hidden="true" />
      ...
      <section className="max-w-6xl mx-auto w-full px-6 py-20 md:py-28">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {outcomes.map((o, i) => (
            ...
            >
              <o.icon className="w-7 h-7 text-teal mx-auto mb-3" strokeWidth={1.5} aria-hidden="true" />
            ...
            <div className="space-y-5">
              {includes.map((item) => (
                <div key={item.title} className="flex items-start gap-3">
                  <CheckCircle2 className="w-5 h-5 text-teal shrink-0 mt-0.5" strokeWidth={1.5} aria-hidden="true" />
                  <div>
                    <p className="font-heading text-base font-bold mb-1">{item.title}</p>
